...
import { useAuditEventStore } from '@/stores/auditEvent';
...
const selectedItems = ref<AuditEvent[]>([]);
...
const toggleSelection = async (id: number) => {
  ...
  selectedItems.value = await auditAPI.search(selectedIDs.value);
};
const handleSearch = (searchQuery: string) => {
  store.searchColumns.keyword = searchQuery;
  store.fetchAuditList();
};
</script>
          ...
          <div class="grid grid-cols-2 gap-8 p-4">
            ...
            <div>
              ...
              <div class="grid grid-cols-5 gap-4 font-bold mb-2">
                <span></span>
                <span class="col-span-2">事件名稱</span>
                <span>讀取方式</span>
                <span></span>
              </div>
              <!-- Item Rows -->
