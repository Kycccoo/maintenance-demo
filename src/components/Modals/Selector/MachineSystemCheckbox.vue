...
import { machineSystemAPI } from '@/api';
...
const toggleSelection = async (id: number) => {
  ...
  selectedItems.value = await machineSystemAPI.search(selectedIDs.value);
};
...
// const detailFactory = ref<MachineSystems | null>(null);
// const openDetailModal = (factory: MachineSystems) => {
    ...
    >
      ...
      >
        <div>
          ...
          <h3 class="pb-2 text-lg font-bold text-black dark:text-white sm:text-2xl">系統選擇</h3>
          ...
          <div class="grid grid-cols-2 gap-8 p-4">
            ...
            <div>
              ...
              <div class="grid grid-cols-4 gap-4 font-bold mb-2">
                <span></span>
                <span class="col-span-2">系統名稱</span>
                <span>廠別</span>
              </div>
              <!-- Item Rows -->
              <div class="space-y-2">
                ...
                <div
                  ...
                  :key="item.id"
                  class="items-center grid grid-cols-4 gap-3 hover:bg-green-50 p-1"
                >
                  ...
                  <span class="col-span-1" @click="toggleSelection(item.id)">{{
                    getFactoryName(item.factoryID ?? 0)
                  }}</span>

                  <!-- <span @click="toggleSelection(item.id)">{{ item.id }}</span> -->
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <span class="col-span-2" @click="toggleSelection(item.id)">{{ item.name }}</span>
                  <span class="col-span-1 text-right" @click="toggleSelection(item.id)">{{
                    getFactoryName(item.factoryID ?? 0)
                  }}</span>
                  <!-- <span @click="toggleSelection(item.id)">{{ item.id }}</span> -->
                </div>
