...
watch(
  ...
  async (newValue) => {
    ...
    selectedItems.value = tagFunctions.filter((item) => newValue.includes(item.id));
  }
);

const selectedIDs = ref<number[]>([]);
const selectedItems = ref<TagFunction[]>([]);
...
const toggleSelection = async (id: number) => {
  ...
  selectedItems.value = tagFunctions.filter((item) => selectedIDs.value.includes(item.id));
};
</script>
