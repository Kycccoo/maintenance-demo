...
<template>
  <transition name="fade">
    <div
      v-if="props.openModal && props.detailFactory"
    ...
    >
      ...
      >
        ...
        <h3 class="pb-2 text-xl font-bold text-black">
          <span class="truncate">工廠詳細資料</span>
        </h3>
        <span class="mx-auto mb-2 inline-block h-1 w-[230px] rounded bg-primary"></span>
        <h2 class="pb-2 text-xl font-bold text-black">
          ID: {{ props.detailFactory.id }} {{ props.detailFactory.name }}【{{
            props.detailFactory.code
          }}】工廠位置：{{ props.detailFactory.groupName }}
        </h2>
        <div class="flex justify-center items-center mb-2">
          <span>{{ props.detailFactory.description }}</span>
        </div>
        ...
        <div class="mb-5">
          <TableMachine :factory="detailFactory" :show-operation="false" />
        </div>
