...
import { addMachineSystemPostErrorMessageMap } from '@/api/errors/machineSystem';
import RequiredSpan from '@/components/Labels/RequiredSpan.vue';
import { useToast } from 'vue-toast-notification';
import { GlobalToastProps } from '@/types/Toast';
import {
  type MachineSystemsError,
  InitMachineSystemsError
} from '@/types/errors/MachineSystemsError';
...
const createMachineSystems = ref<MachineSystems>(structuredClone(InitMachineSystems));
const createError = ref<MachineSystemsError>(structuredClone(InitMachineSystemsError));
...
const closeCreateModal = () => {
  createMachineSystems.value = structuredClone(InitMachineSystems);
  createError.value = structuredClone(InitMachineSystemsError);
  emit('do-close');
};
...
const handleCreate = async () => {
  ...
  if (!createMachineSystems.value.name) {
    $toast.error('工廠名稱必填');
    createError.value.name = '工廠名稱必填';
    return;
  }

  if (!createMachineSystems.value.factoryID) {
    $toast.error('代號必填');
    createError.value.factoryID = '代號必填';
    return;
  }

  try {
    const factory: MachineSystems = await store.create(createMachineSystems.value);
    createMachineSystems.value.id = factory.id;
    store.addItem(createMachineSystems.value);

    try {
      await store.updateOrder(createMachineSystems.value.id, createMachineSystems.value.order);
    } catch (error) {
      console.error(error);
      $toast.error('排序更新失敗');
    }
    ...
    if (error.response?.data?.errors) {
      ...
      errorCodes.forEach((factoryID) => {
        if (
          ...
          createError.value.name = addMachineSystemPostErrorMessageMap[factoryID];
        } else if (
          [
          ...
          createError.value.factoryID = addMachineSystemPostErrorMessageMap[factoryID];
        } else if (
          [
          ...
          createError.value.description = addMachineSystemPostErrorMessageMap[factoryID];
        } else {
          createError.value.general =
            addMachineSystemPostErrorMessageMap[factoryID] || `未知錯誤: ${factoryID}`;
        }
      });
    } else {
    ...
    >
      ...
      >
        ...
        <div class="flex justify-center items-center gap-2 pb-2">
          ...
          <h3 class="text-xl font-bold text-black dark:text-white sm:text-2xl">新增設備系統</h3>
        </div>
        <span class="mx-auto mb-6 inline-block h-1 w-[140px] rounded bg-primary"></span>
        <form class="text-left">
          <div class="mb-4">
            <label class="mb-2.5 block font-medium text-black dark:text-white"
              ><RequiredSpan />設備系統名稱</label
            >
            <div class="relative">
              <input
                v-model="createMachineSystems.name"
                required
                type="text"
                placeholder="請輸入設備系統名稱"
                :class="`border ${
          ...
          <div class="flex gap-2">
            <!-- <div class="mb-4">
              ...
              <div class="relative">
                <input
                  v-model="createMachineSystems.factoryID"
                  required
                  type="text"
                  placeholder="請輸入代號"
                  :class="`border ${
              ...
              <p v-if="createError.factoryID" class="text-sm text-red-500 mt-1">*{{ createError.factoryID }}</p>
            </div> -->
            <div class="mb-4">
              <label class="mb-2.5 block font-medium text-black dark:text-white">
                <RequiredSpan /> 工廠選擇
            ...
            <div class="mb-4">
              ...
              <div class="relative">
                <input
                  v-model="createMachineSystems.order"
                  required
                  value="0"
                  type="number"
                  placeholder="請輸入排序"
          ...
          <div class="mb-4">
            ...
            <div class="relative">
              <textarea
                v-model="createMachineSystems.description"
                rows="4"
                placeholder="請輸入描述"
                :class="`border ${
