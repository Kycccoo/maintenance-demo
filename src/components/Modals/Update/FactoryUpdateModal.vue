...
const updateFactory = ref<Factory>(props.factory);
...
onMounted(() => {
  ...
  updateFactory.value = JSON.parse(JSON.stringify(props.factory));
  store.getGroupName(updateFactory.value.id).then((res) => {
    updateFactory.value.factoryGroupName = res;
  });
  store.getOrder(updateFactory.value.id).then((res) => {
    updateFactory.value.order = res;
  });
});
...
const handleUpdate = async () => {
  ...
  if (!updateFactory.value.name) {
    $toast.error('工廠名稱必填');
    updateError.value.name = '工廠名稱必填';
    return;
  }

  if (userStore.canCreate('manage_factory')) {
    const result = await updateOrder(updateFactory.value.id, updateFactory.value.order);
    if (!result) {
      $toast.error('工廠更新失敗');
      return;
    }
  ...
  if (userStore.canCreate('manage_factory')) {
    const result_group = await updateGroupName(
      updateFactory.value.id,
      JSON.stringify(updateFactory.value.factoryGroupName)
    );
    if (!result_group) {
      $toast.error('工廠更新失敗');
      return;
  ...
  try {
    await store.update(updateFactory.value);
    store.sync(updateFactory.value);
    updateError.value = structuredClone(initUpdateError);
...
const updateGroupName = async (id: number, groupName: string) => {
  try {
    await store.updateGroupName(id, groupName);
    return true;
  } catch (error) {
    handleOrderError(error);
  }
      ...
      >
        ...
        <div class="flex justify-center items-center gap-2 pb-2">
          ...
          <h3 class="text-xl font-bold text-black dark:text-white sm:text-2xl">
            修改工廠資訊 ID: {{ updateFactory.id }}
          </h3>
        </div>
        <span class="mx-auto mb-6 inline-block h-1 w-[260px] rounded bg-primary"></span>
        <div class="text-left">
          <div class="mb-4">
            <label class="mb-2.5 block font-medium text-black dark:text-white">工廠名稱</label>
            <div class="relative">
              <input
                v-model="updateFactory.name"
                required
                type="text"
                placeholder="請輸入工廠名稱"
                :class="`border ${
          ...
          <div class="flex gap-2">
            <div class="mb-4">
              <label class="mb-2.5 block font-medium text-black dark:text-white">代號</label>
              <div class="relative">
                <input
                  v-model="updateFactory.code"
                  required
                  type="text"
                  placeholder="請輸入代號"
                  :class="`border ${
          ...
          <div class="flex gap-2">
            <div class="mb-4">
              ...
              <div class="relative">
                <input
                  v-model="updateFactory.factoryGroupName"
                  :readonly="!userStore.canCreate('manage_factory')"
                  required
                  type="text"
                  placeholder="請輸入工廠位置"
          ...
          <div class="mb-4">
            ...
            <div class="relative">
              <textarea
                v-model="updateFactory.description"
                rows="4"
                placeholder="請輸入描述"
                :class="`border ${
