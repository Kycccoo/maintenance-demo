...
const handelDelete = async () => {
  if (deleteConfirmText.value === '刪除' && store.selectedIDs.length > 0) {
    await store.batchDestroy(store.selectedIDs);
    store.fetch();
    ...
    >
      ...
      >
        ...
        <p class="mb-5 font-medium">
          即將刪除選取的點位映射 ( <span>總數: </span
          >{{ store.selectedIDs.length }} )<br />請輸入確認文字「刪除」。
        </p>
        <input
